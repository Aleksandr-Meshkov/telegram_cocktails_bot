...
@dp.message_handler(text='Случайный коктейль')
async def get_info_drink(message: types.Message):
    """Отправляет сообщение с полной информацией рандомного напитка."""
    response = get_api(ENDPOINT['url_random'], message.text)
